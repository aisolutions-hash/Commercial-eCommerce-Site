...
export default function Auth() {
  ...
  useEffect(() => {
    ...
    if (errorParam === 'google_email_missing') {
      setError('Google account has no email. Try a different account.');
      return;
    }
    if (token && nameParam && emailParam) {
      setAuth(token, { name: nameParam, email: emailParam });
      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full flex items-center justify-center py-16">
        ...
        >
          ...
          <a
            ...
            className="w-full flex items-center justify-center gap-3 border border-border rounded-full py-3 font-medium hover:bg-muted transition-colors"
          >
