...
export default function Auth() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Simulate auth
    login({ name: isLogin ? 'Demo User' : name, email });
      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full flex items-center justify-center py-16">
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button 
              type="submit"
              className="w-full bg-foreground text-background hover:bg-primary hover:text-black py-3 rounded-full font-bold transition-colors mt-6"
            >
